...
%%pathplan_B20 代码简化 解决两端问题
destination=[5500 1750];      %目标点坐标3090,4420,5750,7080,8410
obstacle_x=[4000 6000 6000 6000];%
obstacle_y=[6000 3500 6000 8500];%
%current_position=[5600 ji];
size_obstacle=size(obstacle_x);        %障碍物个数
...
for i=1:num_obstacle
    dis_line=dot_line(k,b,[obstacle_x(i),obstacle_y(i)]);
    if (dis_line<robot_width+radius || dis_line==robot_width+radius)&&(obstacle_x(i)<max(destination(1),current_position(1))+robot_width+radius&&obstacle_x(i)>min(destination(1),current_position(1))-robot_width+radius&&obstacle_y(i)<max(destination(2),current_position(2))+robot_width+radius&&obstacle_y(i)>min(destination(2),current_position(2))-robot_width+radius)

        in_obstacle(count+1)=obstacle_x(i);
        in_obstacle(count+2)=obstacle_y(i);
        count=count+2;
    end
end
...
if (count==0)       %(k==inf||k==-inf)||
    ...
    if count==2 %一个障碍物
        %当障碍物在两点连线上方时
        if (in_obstacle(2)>(k*in_obstacle(1)+b))||(k==inf&&destination(1)>obstacle_x(i))
            %先进行选取四点坐标
            ...
            %检测两两坐标连线上是否会遇到障碍物
            for i=1:num_obstacle
                 if (obstacle_x(i)~=in_obstacle(1))&&(obstacle_x(i)<max(vertices(1,1),vertices(1,2))&&obstacle_x(i)>min(vertices(1,1),vertices(1,2)))&&(obstacle_y(i)<((vertices(2,2)-vertices(2,1))/(vertices(1,2)-vertices(1,1))*obstacle_x(i)+vertices(2,2)-(vertices(2,2)-vertices(2,1))/(vertices(1,2)-vertices(1,1))*vertices(1,2)+robot_dis_obs))&&(obstacle_y(i)>((vertices(2,2)-vertices(2,1))/(vertices(1,2)-vertices(1,1))*obstacle_x(i)+vertices(2,2)-(vertices(2,2)-vertices(2,1))/(vertices(1,2)-vertices(1,1))*vertices(1,2)-robot_dis_obs))||(obstacle_x(i)~=in_obstacle(1))&&(obstacle_x(i)<max(vertices(1,3),vertices(1,4))&&obstacle_x(i)>min(vertices(1,3),vertices(1,4)))&&(obstacle_y(i)<((vertices(2,4)-vertices(2,3))/(vertices(1,4)-vertices(1,3))*obstacle_x(i)+vertices(2,4)-(vertices(2,4)-vertices(2,3))/(vertices(1,4)-vertices(1,3))*vertices(1,4)+robot_dis_obs))&&(obstacle_y(i)>((vertices(2,4)-vertices(2,3))/(vertices(1,4)-vertices(1,3))*obstacle_x(i)+vertices(2,4)-(vertices(2,4)-vertices(2,3))/(vertices(1,4)-vertices(1,3))*vertices(1,4)-robot_dis_obs))       
                    in_obstacle(count+1)=obstacle_x(i);
                    in_obstacle(count+2)=obstacle_y(i);
                    count=count+2;
                    break;
        ...
        if k*max(obstacle_x)+b<min(obstacle_y)
            temp_position1=[max(obstacle_x)+x_offset,min(obstacle_y)+y_offset];
            temp_position2=[max(obstacle_x)-x_offset,min(obstacle_y)+y_offset];
            ...
            temp_position1=[max(obstacle_x)+x_offset,max(obstacle_y)-y_offset];
            temp_position2=[max(obstacle_x)-x_offset,max(obstacle_y)-y_offset];
            if norm(current_position-temp_position1)<norm(current_position-temp_position2)
                temp1=temp_position1;
                temp2=temp_position2;
            else
